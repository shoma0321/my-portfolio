...
function init() {
    ...
    colorList.push(['#000', '#000', '#000', '#000', '#000']);//重ねる波線の色設定
...
function particleTextAnime() {
    var pathname = window.location.pathname; // 現在のページのURLのパスを取得
    var textToShow;

    if (pathname.endsWith("index.html")) {
        textToShow = "Shoma's Portfolio";
    } else if (pathname.endsWith("thanks.html")) {
        textToShow = "Thank you";
    } else {
        textToShow = "Shoma's Portfolio"; // デフォルトのテキスト
    }

    $("#particle").particleText({
        text: textToShow, // 表示させたいテキスト。改行の場合は<br>追加
        colors: ["#da6a0b"], // パーティクルの色を複数指定可能
...
$('.slider').slick({
	autoplay: true,//自動で動き出すかどうか
	infinite: true,//スライドをループさせるかどうか
	speed: 500,
	slidesToShow:3,//画面に表示する数
	slidesToScroll: 1,
